...
>listOf p sep = p `thn` many (token sep `xthn` p) `build` cons
...
>word = spot isAlpha ^^^ many (spot isAlphaNum) ^^^ many (token '\'') >>> cons_
...
>number = spot isDigit ^^^ many (spot isDigit) >>> mknum . cons
> where mknum [] = 0
